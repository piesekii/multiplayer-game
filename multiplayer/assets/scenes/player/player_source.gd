...
@onready var cs_standing: CollisionShape3D = $cs_standing
...
const CROUCH_JUMP_ADD = CROUCH_TRANSLATE * 0.5
...
func _handle_crouch(delta) -> void:
	...
	cs_standing.shape.height = _original_capsule_height - CROUCH_TRANSLATE if is_crouched else _original_capsule_height
	cs_standing.position.y = cs_standing.shape.height / 2
